...
const generateQr = async () => {
  if (!props.user?.qr_auth_token) {
    qrCodeUrl.value = '';
    return;
  }
  try {
    qrCodeUrl.value = await QRCode.toDataURL(props.user.qr_auth_token, { width: 300, margin: 2 });
  } catch (err) {
...
watch(() => props.user?.qr_auth_token, generateQr, { immediate: true });
