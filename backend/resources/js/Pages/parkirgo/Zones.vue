...
export default {
  ...
  data() {
    return {
      ...
      editingZone: null,
      editingTariff: null,
      zoneForm: { code: '', name: '', city: '', capacity_motor: 0, capacity_car: 0, qris_payload: '', status: 'active' },
      tariffForm: { zone_id: null, vehicle_type_id: null, vehicle_type: '', pricing_type: 'flat', payment_timing: 'entry', base_minutes: 60, base_rate: 0, increment_minutes: 60, increment_rate: 0, daily_max_rate: 0, grace_period_minutes: 15, rounding_minutes: 0 },
    };
  },
  methods: {
    openZone(z) {
      if (z) { this.editingZone = z; this.zoneForm = { code: z.code, name: z.name, city: z.city || '', capacity_motor: z.capacity_motor || 0, capacity_car: z.capacity_car || 0, qris_payload: z.qris_payload || '', status: z.status }; }
      else { this.editingZone = null; this.zoneForm = { code: '', name: '', city: '', capacity_motor: 0, capacity_car: 0, qris_payload: '', status: 'active' }; }
      this.showZoneModal = true;
    },
    saveZone() {
      if (this.editingZone) {
        router.post(route('parkirgo.zones.update', this.editingZone.id), this.zoneForm, { preserveScroll: true, onSuccess: () => this.showZoneModal = false });
      } else {
        router.post(route('parkirgo.zones.store'), this.zoneForm, { preserveScroll: true, onSuccess: () => this.showZoneModal = false });
      }
    },
    deleteZone(z) {
    ...
    openTariff(t) {
      ...
      if (t) this.tariffForm = { zone_id: t.zone_id, vehicle_type_id: t.vehicle_type_id, vehicle_type: t.vehicle_type, pricing_type: t.pricing_type, payment_timing: t.payment_timing, base_minutes: t.base_minutes, base_rate: t.base_rate, increment_minutes: t.increment_minutes, increment_rate: t.increment_rate, daily_max_rate: t.daily_max_rate, grace_period_minutes: t.grace_period_minutes, rounding_minutes: t.rounding_minutes };
      else this.tariffForm = { zone_id: this.zones[0]?.id || null, vehicle_type_id: null, vehicle_type: 'motor', pricing_type: 'flat', payment_timing: 'entry', base_minutes: 60, base_rate: 0, increment_minutes: 60, increment_rate: 0, daily_max_rate: 0, grace_period_minutes: 15, rounding_minutes: 0 };
...
<template>
  <Layout>
            ...
            <BCard no-body class="h-100">
              <BCardBody>
                ...
                <div class="d-flex gap-3 text-center">
                  <div><small class="text-muted">Motor</small><br /><strong>{{ z.capacity_motor || '-' }}</strong></div>
                  <div><small class="text-muted">Mobil</small><br /><strong>{{ z.capacity_car || '-' }}</strong></div>
    ...
    <BModal v-model="showZoneModal" :title="editingZone ? 'Edit Zona' : 'Tambah Zona'" hide-footer>
      ...
      <div class="row mb-2">
        <div class="col-6"><label class="form-label">Kapasitas Motor</label><input v-model.number="zoneForm.capacity_motor" type="number" class="form-control" /></div>
        <div class="col-6"><label class="form-label">Kapasitas Mobil</label><input v-model.number="zoneForm.capacity_car" type="number" class="form-control" /></div>
      </div>
    ...
    <BModal v-model="showTariffModal" :title="editingTariff ? 'Edit Tarif' : 'Tambah Tarif'" hide-footer>
      ...
      <div class="mb-2"><label class="form-label">Jenis Kendaraan</label><input v-model="tariffForm.vehicle_type" class="form-control" placeholder="motor/mobil/bus/truk" /></div>
      <div class="row mb-2">
